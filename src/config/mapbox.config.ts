...
export const MAPBOX_ACCESS_TOKEN = API_CONFIG.MAPBOX_TOKEN || 'pk.eyJ1Ijoic3VkaGFrYXJwb3VsIiwiYSI6ImNtaXp0ZmFrNTAxaTQzZHNiODNrYndsdTAifQ.YTMezksySLU7ZpcYkvXyqg';

// Default map settings
export const MAP_CONFIG = {
  // Default center (India)
  DEFAULT_CENTER: [77.2090, 28.6139] as [number, number], // Delhi coordinates
  DEFAULT_ZOOM: 10,
...
export const initializeMapbox = async () => {
  try {
    ...
    if (MAPBOX_ACCESS_TOKEN && MAPBOX_ACCESS_TOKEN !== 'YOUR_MAPBOX_ACCESS_TOKEN_HERE') {
      ...
      console.warn('Mapbox access token not configured. Please set MAPBOX_ACCESS_TOKEN in mapbox.config.ts');
    }
  } catch (error) {
